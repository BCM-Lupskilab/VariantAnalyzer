...
class MapUrlIn(object):
    ...
    def map_app(self):
        ...
        if not base._acfe_match.match(self.application):
            print "base._acfe_match.match(self.application)"
            raise HTTP(
        ...
        if self.application not in routers and \
            ...
            print "elf.application not in routers and (self.application != THREAD_LOCAL.routes.default_application or self.application == 'welcome')"
            raise HTTP(
                400, THREAD_LOCAL.routes.error_message % 'invalid request',
    ...
    def map_controller(self):
        ...
        if not self.router._acfe_match.match(self.controller):
            print "not self.router._acfe_match.match(self.controller)"
            raise HTTP(
                400, THREAD_LOCAL.routes.error_message % 'invalid request',
    ...
    def map_function(self):
        ...
        if not self.router._acfe_match.match(self.function):
            print "not self.router._acfe_match.match(self.function)"
            raise HTTP(
                400, THREAD_LOCAL.routes.error_message % 'invalid request',
                web2py_error='invalid function')
        if self.extension and not self.router._acfe_match.match(self.extension):
            print "self.extension and not self.router._acfe_match.match(self.extension):"
            raise HTTP(
                400, THREAD_LOCAL.routes.error_message % 'invalid request',
    ...
    def validate_args(self):
        ...
        for arg in self.args:
            if not self.router._args_match.match(arg):
                print "not self.router._args_match.match(arg)"
                raise HTTP(
                    400, THREAD_LOCAL.routes.error_message % 'invalid request',
